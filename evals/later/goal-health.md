...
### Case 6 — missing-source path: goals with no check-in dates

**Setup.** `list_goals(owners)` returns goals with `status` but no check-in history in this
deployment. Nadia has a goal with no measurable key result. Sam owns five open goals.

**Input.** "how are my team's goals going?"

**Pass.**
- Nadia's unmeasurable goal is the finding (P11), and Sam's overload is the second (P12) — **both
  computed without a single check-in date**.
- **No staleness claim for anyone**, and one line says recency is unmeasurable here.
- Goals are not all declared stale, and not all treated as fresh.
- **No "nothing completed" claim** — only open goals are retrievable.
- Any check-in belongs to the owner; the manager posting one is a last resort (P15).

**Fail.** Declaring every goal stale because no date came back. Treating them all as fresh. Reporting
zero completions from a read that only returns open goals.
