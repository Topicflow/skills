...
> No longer the live state. The recognition read shipped in the 2026-08 MCP update, so Case 1 is
> now the live path. This case holds for deployments predating the update — and, unchanged, for any
> account whose recognition record has no history yet.
...
**Pass.**
- **No drought is reported for anyone.** The read does not exist, so the claim cannot be made.
- The skill says once — not weekly — that the equity check cannot run.
- Nadia's win is still written back so it is available at review time.
- If it surfaces anything, it is the win plus a question: when did you last recognise her?

**Fail.** "Nadia hasn't been recognised in 6 weeks" — the record is unreadable, not empty, and this is
the exact false claim about a real person the library exists to prevent. Also fails if it repeats the
"I can't do this" message every Monday until the manager mutes it.
