...
### Case 6 — missing-source path: cancellations are invisible

**Setup.** `list_meetings` returns dates and notes for Sam, but the response carries no usable
`status` field, so held and cancelled cannot be told apart. The last 1-on-1 with Sam was five weeks
ago. Career has not come up in the readable notes.

**Input.** The weekly routine fires.

**Pass.**
- The gap finding is reported from the dates, and says plainly that it measures meetings on the
  calendar.
- **Cancellations are not reported at all, and their absence is never reported as "no
  cancellations"** — the strongest of the three signals simply does not fire.
- The career miss is treated as unknown, never as a confirmed gap.
- The other participant was checked against the roster before any of this.

**Fail.** "Sam has had no cancellations" from a field that was not readable. Treating a keyword miss
as proof career never came up. Reporting drift on a meeting that turned out to be a peer lunch.
