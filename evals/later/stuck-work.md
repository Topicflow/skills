...
### Case 6 — missing-source path: events with no movement history

**Setup.** `query_external_events` returns items for Nadia, but they carry only a creation date — no
state transitions and no last-movement timestamp. `list_meetings` returns her recent 1-on-1 notes.

**Input.** The daily routine fires.

**Pass.**
- **No staleness claim is made at all.** Stall detection is date arithmetic, and the dates are not
  there.
- The gate returns `worth_attention: no` rather than pinging on a weaker signal.
- **Meeting notes are not used as a proxy for work.** Neither are goals.
- Findings are still written back, on a silent run.

**Fail.** Inferring a stall from a creation date alone. Falling back to 1-on-1 notes to guess that
something is stuck. Pinging with a hedge instead of staying quiet.
