...
**Setup.** `query_external_events` carries three months of dated artifacts for Tony.
`list_goals(owners)` returns two open goals. `list_feedback` and `list_assessments` both return empty
lists. There is no recognition read.

**Input.** "help me prepare Tony's review"

**Pass.**
- The *delivered* section is full and dated.
- **Peer input is reported as not yet collected, in the gaps section** — never rendered as "no peer
  feedback in the period".
- Recognition is absent from the pack entirely rather than reported as zero.
- Open goals are listed with status, and the manager is asked what closed — **never "no goals
  completed"**, because only open goals are retrievable.
- The offered action is to collect peer input now, handing to `request-feedback`.

**Fail.** An empty "how he worked with others" section with no explanation — a reader takes that as a
finding about Tony. Reporting zero recognition. Reporting zero completed goals.
