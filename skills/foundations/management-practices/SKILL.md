...
description: The checkable management rules (P1-P17) behind every skill in this library — 1-on-1s, feedback, recognition, goals, career, coaching. Use when checking whether a draft agenda, feedback, recognition, or goal is good enough to send, when another skill needs the rule text, or when the user asks what good management practice says about a situation.
---

# Management practices — the checks
...
showing them to the manager.

Narrative, citations, and source URLs: [management-practices.md](../../../references/management-practices.md).
...
then show the manager. A draft that fails a check is not shown with a caveat — it is fixed.

**As an answer to a question.** When the manager asks how to handle something, answer with
the rule and one sentence of why, not with a lecture. Name the rule number so they can
look it up.

Never quote rule numbers *at* the manager in normal output. "P5" belongs in your reasoning;
